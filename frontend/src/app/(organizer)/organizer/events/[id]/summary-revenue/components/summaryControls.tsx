...
// Sample show times data (you can replace this with your actual data)
const sampleShowTimes = [
  { date: "20 Tháng 03, 2025", time: "00:00", isSelected: true },
  { date: "21 Tháng 03, 2025", time: "00:00" },
  { date: "22 Tháng 03, 2025", time: "00:00" },
  { date: "23 Tháng 03, 2025", time: "00:00" },
]

export const SummaryControls = ({ eventTitle }: SummaryControlsProps) => {
  ...
  const handleShowTimeChange = (selectedShowTime: any) => {
    // Update the current show time based on selection
    setCurrentShowTime({
      date: selectedShowTime.date,
      time: selectedShowTime.time,
      year: "2024", // You might want to extract this from the selectedShowTime
    })
  }

  return (
    <div className="mb-4">
      {/* Header flex giữa Title và Button */}
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-[#0C4762]">{eventTitle}</h1>
          <p className="text-sm text-[#51DACF] pt-2">
            {currentShowTime.time}, {currentShowTime.date}, {currentShowTime.year}
          </p>
        </div>

        <button
          className="flex items-center gap-2 border-2 border-[#0C4762] text-[#0C4762] bg-white px-4 py-2 rounded-lg hover:bg-[#f0f8ff] transition"
      ...
      <div className="flex gap-2">{/* Bạn có thể thêm button/filter ở đây nếu muốn sau */}</div>
