...
export class KafkaService implements OnModuleInit {
  ...
  emitUserChangeStatusEvent(user: {userId: string, status: string}) {
    this.client.emit('user_change_status', {
      event: 'USER_CHANGE_STATUS',
      userId: user.userId,
      status: user.status,
      timestamp: new Date().toISOString(),
    });
    Logger.log(`Emitted user_change_status event: ${JSON.stringify(user.userId)}`)
  }
}