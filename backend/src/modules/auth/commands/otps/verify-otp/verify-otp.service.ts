...
import { OTPConstants } from 'src/shared/constants/otp.constants';
...
export class VerifyOTPService {
  ...
  async execute(
    ...
    // Validate command
    if (!command.type) {
      return Err(new Error('OTP type is required'));
    }
    // Validate email
    ...
    if (!otpData) {
      ...
      if (remaining_attempts <= 0) {
        ...
        return Err(new Error('Maximum attempts reached. Please request a new OTP.'));
      }
      await this.authRepository.incrementOTPAttempts(command.request_token);
      return Err(new Error('Invalid or expired OTP. You have ' + remaining_attempts.toString() + ' attempts left.'));
    }
    ...
    if (command.type === OTPType.REGISTER) {
      ...
      if (!temp) {
        return Err(new Error('User not found in local storage'));
      }
    ...
    return Err(new Error('Invalid OTP type'));
  }
}